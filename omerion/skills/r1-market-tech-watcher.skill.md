...
spec: agents.r1_market_tech_watcher.managed_agent:spec
# TRACK (Real Estate Market/Tech Watcher)
# Local APScheduler must NOT fire this — the cloud runtime owns triggering.
triggers:
  - managed_agent_cron
events_consumed: []
events_emitted:
  - rd.insight.created
hitl: false
model_tier: SONNET          # Single-model managed agent. Sonnet for RE-nuance tagging.
                            # (Haiku is a valid cost lever — tagging is cheap classification.)
...
A secret that reads back as `ANTHROPIC_SECRET_PLACEHOLDER_...` at egress is NORMAL — substituted
on outbound requests. Do NOT fail merely because a vault secret looks like a placeholder string
when read locally; fail only when the secret name is **empty / unbound**.
...
estimated_priority`, optional `raw_content`, optional `metadata` (service_package_tag etc.).
...
for richer RE-context summaries on nuanced signals. (The old Haiku→Sonnet escalation hybrid
is not possible in a single-model managed agent — pick one and keep the YAML + this frontmatter
in agreement.)
